...
from RDXMUSIC.mongo.notesdb import *
from RDXMUSIC.utils.notes_func import GetNoteMessage, exceNoteMessageSender, privateNote_and_admin_checker
from RDXMUSIC.utils.yumidb import user_admin
...
async def PrivateNote(client, message):
    ...
    if len(message.command) >= 2:
        ...
        if await is_pnote_on(chat_id):
            await message.reply(
                "Your notes are currently being sent in private. RDXMUSIC will send a small note with a button which redirects to a private chat.",
                quote=True
            )
        else:
            await message.reply(
